...
% ============================================================================
%  Revision-highlighting toggle (resubmission to IEEE Access).
%  HIGHLIGHTED build: revised text = red glyphs on a yellow background.
%  CLEAN build: \rev is a no-op. Flip the single \highlighttrue/\highlightfalse
%  line below and recompile to switch between the two deliverables.
%  Units/inline-math inside a \rev{...} must be wrapped in \mbox{...} so the
%  soul highlighter treats them as one opaque box (soul cannot letter-space
%  math). \reveq highlights a whole new/changed display equation in red;
%  \revcap marks a new float caption.
% ============================================================================
\usepackage{soul}
\definecolor{revhl}{rgb}{1,1,0.30}
\sethlcolor{revhl}
\newif\ifhighlight
\highlightfalse   % <<< \highlighttrue for HIGHLIGHTED build, \highlightfalse for CLEAN build
\ifhighlight
  \newcommand{\rev}[1]{{\color{red}\hl{#1}}}
  \newcommand{\reveq}[1]{{\color{red}#1}}
\else
  \newcommand{\rev}[1]{#1}
  \newcommand{\reveq}[1]{#1}
\fi
% soul natively protects the contents of an \mbox but breaks on bare
% cross-references, citations, and math (and \soulregister triggers a \protect
% recursion in this class). Rule for authored \rev text: wrap every \ref, \cite,
% \eqref, \SI/unit macro, and inline $math$ in \mbox{...} so soul treats each as
% one opaque, still-highlighted box.
...
% Double-column (figure*/table*) top floats default to at most 0.7 of the page,
% which forces a full-width figure to defer even when a same-page single-column
% table is accepted; raise the double-column allowances so a figure* can sit at
% the top of the intended page alongside a column table below it.
...
123-node feeder. \rev{The design establishes voltage feasibility; feeder thermal reinforcement and
independent converter current limits are outside the planning model. Its sensitivity to
the candidate-pool size and to a load-object uncertainty model with group-shared and
idiosyncratic components, and the effect of converter switching
and conduction losses on the net benefit, are additionally quantified.}
\end{abstract}
...
\item[$t$] Hour index of the daily profile \mbox{($t=0,\dots,23$)}.
...
\item[$Q_i(t)$] Dispatched reactive output of the \mbox{D-STATCOM} at bus $i$ (kvar).
...
\rev{The novelty is not the line-to-line
quantity itself, which is standard, but its \emph{consistent} enforcement as the
decision metric across every planning stage---candidate screening, support
selection, capacity sizing, and hourly dispatch---together with a quantified
measure of what a connection-inappropriate choice costs in installed capacity.}
...
\midrule
\cite{statcom2025cmes}        & IEEE 14-bus system with SCIG wind & No  & Bus magnitude\newline (not explicitly reported) & Yes & No \\
\cite{pvdstatcom2025fixedvar} & Balanced radial distribution      & No  & Bus magnitude\newline (not explicitly reported) & Yes & Yes \\
\cite{pareto2024dgcomp}       & Balanced radial distribution      & No  & Bus magnitude\newline (not explicitly reported) & Yes & Yes \\
\cite{estatcom2024egwo}       & Radial distribution, load unc.\   & No  & Bus magnitude\newline (not explicitly reported) & Yes & No \\
\cite{dar2024ccmpc}           & Active distribution network       & Not explicitly reported & Bus magnitude\newline (not explicitly reported) & No (operation) & Yes \\
\cite{zhan2024ofo}            & Distribution feeder               & Not explicitly reported & Bus magnitude\newline (not explicitly reported) & No (operation) & Yes \\
\cite{hua2025safedrl}         & Distribution network              & Not explicitly reported & Bus magnitude\newline (not explicitly reported) & No (operation) & Yes \\
\cite{zheng2025marl}          & Active distribution network       & Not explicitly reported & Bus magnitude\newline (not explicitly reported) & No (operation) & Yes \\
...
accompanying archived deposit (see Data Availability).
...
interpreted as the available reactive-power rating. Equation~\eqref{eq:qbound} restricts the
device to capacitive (injection-only) reactive support. \rev{Two extensions are distinct and
both are left to future work: bidirectional reactive operation at \mbox{$P_i=0$}
(\mbox{$-\Qmax_i\le Q_i\le\Qmax_i$}), and full \mbox{$P$--$Q$} four-quadrant operation subject
to an apparent-power limit, which requires an energy-storage- or DC-source-backed converter.} Because $\mathrm{kW}=0$,
...
$V_{\!\downarrow}=\SI{0.95}{pu}$. \rev{In the balanced approximation the current scales as
\mbox{$|Q_i|/(\sqrt3\,V_{LL,i})$}, so a device operating within \mbox{$\Qmax_i$} at a
line-to-line voltage of at least \mbox{\SI{0.95}{pu}} stays within \mbox{$\Iben_i$}. This
is a reference relation for balanced conditions only, not a guarantee for unbalanced
operation, whose per-conductor currents are evaluated with \mbox{\eqref{eq:iconv}}.} The
check is therefore a post-design reporting statement, \emph{not} a binding constraint
and not a contribution. Consequently the
...
\begin{table}[!t]
	...
	\begin{tabularx}{\columnwidth}{@{}>{\centering\arraybackslash}p{0.85cm} >{\centering\arraybackslash}p{0.85cm} C C@{}}
		\toprule
...
pool under the stated search protocol; \rev{an explicit candidate-pool sensitivity
(Section~\mbox{\ref{sec:poolsens}}) confirms that widening the pool to fifteen or twenty
buses changes neither the minimum device count found under the stated protocol nor the
best known least-capacity support.}
...
size~8 over the three free ratings; with Sobol initialization the solver rounds the
population up to the next power of two, giving 32 vectors per generation; Stage~A 10
generations, Stage~B 18 generations, plus local polishing),
...
\centering
\caption{Representative D-STATCOM Design (Verified-Feasible Three-Support Found
Under the Stated Enumeration Budget): Per-Bus Reactive Rating and Peak Operating
Current}
\label{tab:design}
\renewcommand{\arraystretch}{1.2}
...
\bottomrule
\multicolumn{5}{@{}p{0.97\linewidth}}{\footnotesize $|I_i|$ is the peak per-phase
...
\bottomrule
\multicolumn{5}{@{}p{0.97\linewidth}}{\footnotesize $\hat v_{\mathrm{best}}$ is the
...
\SI{0.1}{\kvar}. The slack above the reserve is small precisely because the
least-capacity feasible vector was selected; here, \SI{0.95}{pu} is the lower
...
(\SIlist{49.8;31.6;50.5}{A}); these are operating currents under the device model of
Section~\ref{sec:model}. The direct minimum-count search
...
so $\hat n=3$ is the smallest feasible count found.
...
(Section~\ref{sec:model}). The total equals \SI{1041.3}{\kvar}.
...
$v_{\mathrm{best}}$). Fig.~\ref{fig:w4bars} summarizes the largest
...
\begin{figure}[!t]
	...
	\caption{Largest worst line-to-line minimum $\hat v_{\mathrm{best}}$
		\eqref{eq:vbest} found for one- and two-device supports; neither reaches the
		\SI{0.9562}{pu} target. The diamond is the three-device \emph{operating} minimum,
...
\label{sec:certify}
\rev{Four} exploratory studies probe the design's sensitivity and isolate the effect of
...
\begin{figure}[!t]
	...
	\caption{\rev{Pool-size and device-count sensitivity over the 10-, 15-, and 20-bus
		pools; bar labels are search successes, not feasible counts. No feasible one- or
		two-device vector was found over all 37 eligible buses.}}
\label{fig:poolsens}
\end{figure}

\emph{\rev{Candidate-pool sensitivity.}}\label{sec:poolsens} \rev{Because the ten-bus pool is itself a
screening choice, its sufficiency is tested directly (Fig.~\mbox{\ref{fig:poolsens}}),
separating two questions: the minimum device count, and the effect of the pool size
on the least-capacity design. First, extending the count search of
Section~\mbox{\ref{sec:sensitivity}} to \emph{every} one of the 37 eligible three-phase
buses, no voltage-feasible one- or two-device vector was found under the stated
numerical search protocol and the per-bus \mbox{450-kvar} bound; a verified-feasible
three-device design was obtained. This does not certify global minimum cardinality,
since the discrete enumeration of supports and the continuous search inside each
support are distinct. Second, the three-device enumeration was repeated over enlarged
pools of the 10, 15, and 20 weakest buses. Each enlarged pool contains the canonical
support \mbox{$\{735,740,741\}$}, and that design---\mbox{1041.3 kvar}, sized in this
section---was re-evaluated in the pool-sensitivity evaluator and is feasible there
(minimum line-to-line voltage \mbox{0.9565 pu}, maximum \mbox{1.0418 pu}, converged).
It therefore remains the best known three-device design across all three pools. The
independent three-seed search used for the pool study did not recover it, returning
\mbox{$\{711,735,740\}$} at \mbox{1043.1 kvar}; re-running that search on the canonical
support itself yields \mbox{1044.6 kvar} at best, which places the \mbox{1.8-kvar} gap
within the resolution of the stochastic sizing budget rather than in the pool choice.
The number of supports for which a feasible vector was \emph{found} grows from 119 of
120 to 359 of 455 to 852 of 1140 as the pool widens---a search-success count under a
fixed per-support budget, not a count of feasible combinations---while the best known
capacity does not fall. The buses added beyond the initial ten have \emph{higher}
base worst-case line-to-line voltages (mean \mbox{0.916 pu} versus \mbox{0.902 pu} for
the first ten), consistent with the low-voltage cluster already containing the binding
support; no smaller-capacity design was found by extending the pool.}
...
sub-\SI{0.1}{\percent} absolute difference by a small (\SI{151.9}{kWh}) denominator.
...
the band-only coordinated loss-minimizing reference; the reserve-preserving
reference is summarized in Table~\ref{tab:canonical}.}
\label{fig:envelope}
\end{figure*}
...
models here.} At the same nominal three-phase kvar parameter the fixed capacitor
satisfies the operational \numrange{0.95}{1.05}~pu band but fails the
\SI{0.006}{pu} planning reserve, and incurs both the higher daily loss and the
higher voltage unbalance (Section~\ref{sec:vuf}). Section~\ref{sec:cap} isolates
this device-model effect and states its qualifications.

\begin{table}[!t]
\caption{Canonical 24-Hour Comparison of Control Strategies (IEEE 37-Node,
\SI{1041.3}{\kvar} Design; Single Connection-Consistent Evaluator)}
\label{tab:canonical}
\centering
\renewcommand{\arraystretch}{1.2}
\setlength{\tabcolsep}{3pt}
...
ampacity. Worst minima occur at the peak hour and maxima at light load; all binding
terminals are line-to-line (the binding pair is phases $c$--$a$ at bus~740).} \\
\end{tabularx}
\end{table}
...
\caption{Maximum line-to-line voltage at the same nominal kvar parameter (fixed
converter and capacitor) versus the band-only coordinated reference; all strategies
stay below \SI{1.05}{pu} over the evaluated light-load hours.}
\label{fig:maxv}

\vspace{2ex}
\includegraphics[width=\textwidth]{Fig7.pdf}
\caption{Hourly minimum line-to-line voltage of the base feeder (worst case
\SI{0.8986}{pu}) and of the band-only coordinated loss-minimizing reference (worst
case \SI{0.9526}{pu}); the other strategies are listed in
...
\caption{Maximum line-to-line voltage at the same nominal kvar parameter for a fixed
capacitor and a constant-$Q$ converter over the daily profile.}
\label{fig:cap}
\end{minipage}\hfill
\begin{minipage}[t]{0.48\textwidth}
\centering
\includegraphics[width=\linewidth]{Fig9.pdf}
\caption{Aggregate hourly dispatched reactive power $\sum_i Q_i(t)$ for fixed full
output, the local Volt/VAR proxy, and the band-only coordinated reference; the
controlled strategies inject only what each hour requires.}
\label{fig:droop}
\end{minipage}
\end{figure*}
...
\begin{table}[!t]
	\caption{\rev{Maximum Sequence VUF and NEMA Line-Voltage-Unbalance Rate (LVUR) over the
		Deterministic 24-Hour Profile (IEEE 37-Node, \mbox{\SI{1041.3}{\kvar}} Design)}}
	\label{tab:vuf}
	\centering
	\renewcommand{\arraystretch}{1.2}
	\setlength{\tabcolsep}{4pt}
...
\caption{\rev{Load-object Monte-Carlo with group-shared and idiosyncratic components
(\mbox{$N=2000$}) at the stressed spread \mbox{$(\sigma_g,\sigma_\ell)=(0.06,0.08)$}.
(a) Minimum line-to-line voltage under the reactive local proxy; the band is breached
in \mbox{2.75\%} of draws. (b) The three controls coincide on feasibility but differ
in mean injected reactive power and mean feeder loss.}}
\label{fig:mc}
\end{figure*}
\rev{Three} post-design screens probe the planning assumptions. First, a near-peak
...
\SI{95}{\percent} CI \SIrange{4.2}{11.4}{\percent}); \SI{93}{\percent} of
...
sensitivity, not a full stochastic validation---\rev{the load-object
Monte-Carlo described next relaxes the single-multiplier restriction,
while correlated temporal and DER-coupled scenarios remain future work}---so it
bounds the peak-load capacity margin rather than guaranteeing robustness.

\rev{Second, a load-object Monte-Carlo with group-shared and idiosyncratic components
relaxes the single-multiplier restriction. Each of \mbox{$N=2000$} draws multiplies load
object \mbox{$\ell$} by}
\begin{equation}
\reveq{m_\ell = 1 + e_{g(\ell)} + e_\ell,\qquad
e_g\sim\mathcal N(0,\sigma_g^2),\ \ e_\ell\sim\mathcal N(0,\sigma_\ell^2)}
\label{eq:mcerr}
\end{equation}
\rev{with a group-shared term \mbox{$e_g$}, drawn independently for each of three groups,
and a per-object idiosyncratic term \mbox{$e_\ell$}. The group index \mbox{$g(\ell)$} is the
first node listed in the load object's bus connection, which is a connection label rather
than a physical phase. On this feeder all 30 load objects are delta-connected; for the 29
single-phase objects the group coincides with the \mbox{$ab$}, \mbox{$bc$}, or \mbox{$ca$}
branch to which the load is connected, whereas the one three-phase object is assigned to a
single group as a whole. Loads in the same group share \mbox{$e_g$} and are therefore
correlated, and each load object carries one multiplier applied to its \mbox{$P$} and
\mbox{$Q$} together, so the three branches of a three-phase object are not perturbed
separately. Fully phase- and branch-independent variation is therefore not validated here,
and the request for phase-independent variation is addressed only partially; the
object-to-group mapping is given in Supplementary Table~S5. For each draw the base hour is
sampled uniformly (probability \mbox{1/24} per hour) from the 24-hour profile and its load
multiplier \mbox{$\lambda(t)$} is applied; draws use a fixed seed (2026) with
\mbox{$N=2000$} per uncertainty level, multipliers are floored at \mbox{0.05} to exclude
non-physical negative load, and every controller is evaluated on the \emph{same} draw so the
comparison is paired. Over the installed \mbox{$\{735,740,741\}$} design the
connection-voltage band is violated in \mbox{1.95\%} of draws at
\mbox{$(\sigma_g,\sigma_\ell)=(0.04,0.05)$} and \mbox{2.75\%} at the stressed
\mbox{$(0.06,0.08)$}; the worst line-to-line minima are \mbox{0.934} and \mbox{0.925}
pu---the latter \mbox{0.025} pu below the assessment limit---with the fifth-percentile
minimum at or above \mbox{0.955} pu. Over all draws and all monitored three-phase buses the
worst sequence VUF is \mbox{4.85\%} and \mbox{5.23\%} and the worst LVUR \mbox{4.57\%} and
\mbox{5.02\%}, identical for the three controls below; Table~\mbox{\ref{tab:vuf}} reports the
deterministic 24-hour values. The three controls are fixed full output; an open-loop
schedule whose hourly set-points are pre-computed by applying the local Volt/VAR law to the
error-free load of that hour and then held without observing the realized error; and the
reactive local proxy. The schedule is therefore distinct from the coordinated
loss-minimizing reference of Section~\mbox{\ref{sec:coordrefs}} (mean injection
\mbox{1007} versus \mbox{681} kvar). The three controls return the \emph{same} violation
probability and worst voltage. A 400-draw re-run of the same sampling shows why: every
violating draw falls in the evening hours 17--20 (load multiplier \mbox{1.16--1.34}; 18 of
the 27 at hour 19), and in each of them the schedule and the proxy both sit at full output
(\mbox{1041.3 kvar}), so all three controls apply the same injection in the binding tail.
Away from that tail the controls do differ: in \mbox{44--47\%} of the re-run draws the fixed
and proxy minimum voltages differ by more than \mbox{$10^{-6}$} pu, and over the day they
differ in efficiency (mean injected reactive power \mbox{1041}, \mbox{1007}, and \mbox{999}
kvar and mean feeder loss \mbox{123.4}, \mbox{122.0}, and \mbox{121.7} kW at the nominal
spread, Fig.~\mbox{\ref{fig:mc}}). Feasibility in the binding tail is therefore governed by
the installed capacity rather than the control law. The residual violation rate is reported
as observed; the design was sized on a deterministic peak snapshot and is not certified
against a chance constraint.}

Third, a thermal screen confirms that voltage
...
reported against its own derived benchmark (Table~\ref{tab:design}). Line
reinforcement is thus a distinct, complementary planning action, treated separately
from the reactive-support design. \rev{Sizing the reactive support at a thermally
overloaded snapshot is deliberate: \mbox{$\lambda=1.34$} is an upper stress case, and the
capacity reported here is the support required \emph{before} any line reinforcement.
Reconductoring would change both the line impedance and the voltage sensitivities, so
the reactive support would have to be re-optimized after such reinforcement; no
reduction of the required capacity is claimed.}
...
about \SI{23.6}{\percent}. On the corrected metric a compact \SI{1041.3}{\kvar},
...
reported against their derived benchmarks (Table~\ref{tab:design});
...
\begin{figure}[!t]
	...
	\caption{\rev{Net daily energy (feeder-loss saving minus converter loss) versus
			converter loss at rating; the break-even loss (``BE'') is highest for coordinated
			dispatch and lowest for fixed full output.}}
	\label{fig:vscloss}
\end{figure}

\subsubsection{\rev{Converter Losses and the Net Daily Benefit}}
\rev{The loss reductions above are \emph{feeder} losses; a converter also dissipates
switching and conduction losses that scale with its own output and are not captured
by the power flow. To gauge their effect, a three-term device-loss model is applied post hoc to the
24-hour dispatch. For a device of assumed nameplate rating $S_r$ delivering apparent power $S$,}
\begin{equation}
\reveq{P_{\mathrm{loss}}(S)=L_{\mathrm{tot}}\,S_r\!\left(f_{\mathrm{sw}}
+f_a\frac{S}{S_r}+f_b\Big(\frac{S}{S_r}\Big)^{\!2}\right),}
\label{eq:vscloss}
\end{equation}
\rev{with $(f_{\mathrm{sw}},f_a,f_b)=(0.4,0.2,0.4)$ summing to unity, so that
$P_{\mathrm{loss}}(S_r)=L_{\mathrm{tot}}S_r$ and a standby term of
$0.4\,L_{\mathrm{tot}}S_r$ remains at $S=0$. For the loss sensitivity analysis only, each
converter is assigned an assumed nameplate rating of \mbox{$S_r=450$ kVA}. The
reactive-power limits remain those in Table~\mbox{\ref{tab:design}}, summing to
\mbox{1041.3 kvar}. This hardware-rating assumption is not an output of the
placement-and-sizing optimization. Because \mbox{$P=0$} in this post-processing model, the
apparent power is \mbox{$S=|Q|$} in kVA, taken from the hourly dispatch of
Section~\mbox{\ref{sec:operation}}: fixed full output uses the per-device limits of
Table~\mbox{\ref{tab:design}}, and for the controllable laws the hourly total is
apportioned to the devices in proportion to those limits. The coordinated trajectory is
the band-only reference of Section~\mbox{\ref{sec:coordrefs}}; the reserve-preserving
trajectory differs from it in 4 of the 24 hours (by at most \mbox{53.5 kvar}) and is not
used here. \mbox{$S/S_r$} is used as a proxy for the current ratio, which neglects the
terminal-voltage variation. The \mbox{23.6\%} gap of Section~\mbox{\ref{sec:certify}}
compares continuous reactive-capacity designs (\mbox{1287.4} versus \mbox{1041.3 kvar});
if both designs were built from three \mbox{450 kVA} converters their purchased ratings
would be equal, so that gap is not a difference in purchased hardware rating or equipment
cost. The total loss at rating $L_{\mathrm{tot}}$ is swept
from 1 to 2.5\% (Fig.~\mbox{\ref{fig:vscloss}}); no device datasheet is used to
calibrate it, so the sweep is a sensitivity band rather than a manufacturer-specific
figure. Because the coordinated and proxy laws inject far
less reactive energy over the day than fixed full output (daily-mean \mbox{681} and
\mbox{965} versus \mbox{1041} kvar), their converter losses are correspondingly
smaller, so the net daily energy---feeder-loss saving minus converter loss---stays
positive up to a higher converter-loss level. At a representative \mbox{1.5\%} loss
the net daily energy is \mbox{$+189$} kWh for the coordinated reference and
\mbox{$+34$} kWh for the local proxy but \mbox{$-54$} kWh for fixed full output, and
the break-even converter loss is \mbox{2.45\%} coordinated, \mbox{1.64\%} proxy, and
\mbox{1.29\%} fixed. The ordering favours controllable dispatch---the same
controllability that lowers feeder loss also limits the converter's own loss, and
holding full output continuously is the first strategy to turn net-negative.
Controllable dispatch preserves a positive net benefit over a wider range of assumed
converter losses than fixed output, but not over the entire \mbox{1--2.5\%} range: the
proxy turns negative above \mbox{1.64\%} and the coordinated reference above
\mbox{2.45\%}. The absolute figures depend on the assumed loss
split and rating and are reported to establish the sign and ordering of the effect,
not a device-specific efficiency.}
...
\begin{table*}[!t]
	...
	\begin{minipage}[t]{0.485\textwidth}
		...
		\setlength{\tabcolsep}{3pt}
		\footnotesize
		\begin{tabularx}{\linewidth}{@{}>{\hsize=1.62\hsize\raggedright\arraybackslash}X >{\hsize=0.53\hsize\centering\arraybackslash}X >{\hsize=0.85\hsize\centering\arraybackslash}X@{}}
			\toprule
...
active-power-dependent $P$--$Q$ capability curve, \rev{DC-link voltage and energy dynamics}, and
converter dynamics are not modeled, so the constant-$Q$ device is a lower-fidelity
proxy \rev{for a D-STATCOM with its full dynamic behavior; energy-storage-backed operation
with a battery state of charge is a separate extension.}
\emph{\rev{Converter losses.}} \rev{Switching and conduction losses are not captured
by the power flow; a post-hoc device-loss model (Fig.~\mbox{\ref{fig:vscloss}}) moves
the net-benefit break-even to a converter loss of \mbox{1.3--2.5\%} depending on the
control law, and a dispatch that co-optimizes feeder and converter loss is future
work.}
\emph{Uncertainty.} A
single deterministic 24-hour profile scaled by one scalar multiplier is the sizing
basis, so it omits phase-specific, spatial, and load/PV-correlated variation; the absolute
violation magnitudes are illustrative, and a stochastic treatment with correlated,
held-out scenarios is the natural next step. \rev{A load-object Monte-Carlo with
group-shared and idiosyncratic components is reported in Section~\mbox{\ref{sec:robust}}
(\mbox{2--3\%} band violations); fully phase- and delta-branch-independent variation,
correlated temporal and DER-coupled variation, and scenario-based sizing remain open.} The voltage dependence $Q_{\mathrm{cap}}\propto V^2$ in \eqref{eq:cap} is a general
...
load/PV data would broaden external validity. The IEEE 123-node study
(Section~\ref{sec:ieee123}) is a transferability check conditioned on the existing
regulator and capacitor controls, not an independent second validation or a
generalization claim.
\emph{Controller tuning.} The local-proxy knee $(V_\downarrow,V_\uparrow)=(1.00,1.03)$
is a conventional Volt/VAR setting evaluated on the same profile. A
$(V_\downarrow,V_\uparrow,\beta)$ sensitivity is reported in
\rev{Section~\mbox{\ref{sec:certify}}}: the band is held in all \num{27} settings, while the
loss-capture ratio ranges over \SIrange{9.7}{79.7}{\percent}, so the reported capture
is knee-specific. Selecting the knee on held-out data is left for future work, as are
measurement delay, filtering, converter inner-loop dynamics, \rev{switching
harmonics, and output-filter design}. The damping $\beta$ is a numerical coefficient
of the fixed-point iteration and is not a validated controller response time.
...
a line-to-line minimum of \SI{0.9792}{pu}. This is therefore a no-action case \emph{on the undervoltage criterion}: because the
existing regulation already holds the lower limit, no placement or sizing for
undervoltage support is triggered---the honest counterpart to a frozen-regulator
...
On the corrected metric, no one- or two-device support was found that reached the \SI{0.9562}{pu}
acceptance---\rev{neither within the ten-bus pool nor over all \mbox{\num{37}}
eligible three-phase buses}---under the \SI{450}{\kvar} per-bus bound, and enumerating
...
complete network-feasible plan. \rev{Widening the candidate pool to twenty buses
left the minimum device count found under the stated protocol and the best known
least-capacity support unchanged, and a load-object Monte-Carlo with group-shared and
idiosyncratic components (not a fully phase-independent test) violated the band in
\mbox{2--3\%} of draws, so the selection is insensitive to the pool cut-off while the residual
violation rate reflects sizing on a deterministic peak snapshot rather than on a
chance constraint. Once converter switching and conduction losses are included, the
ordering of the control laws is preserved but the net benefit is not unconditional:
the break-even converter loss is \mbox{2.45\%} for the coordinated reference and
\mbox{1.64\%} for the local proxy, while fixed full output is already net-negative at
\mbox{1.5\%}, so controllable dispatch preserves a positive net benefit over a wider
range of assumed converter losses than fixed output, but not over the entire
\mbox{1--2.5\%} range.} Applied with native controls active, the same
evaluator found the regulated IEEE 123-node feeder not undervoltage-limited
(connection-consistent minimum \SI{0.9657}{pu}), a no-action case for undervoltage
support; a residual overvoltage of \SI{1.052}{pu} remains outside the scope of the
capacitive design. Future
...
hourly load multipliers---are publicly available in an archived, citable deposit at
\mbox{\url{https://doi.org/10.5281/zenodo.21535356}} (concept DOI, always resolving to the
latest version). \rev{The version underlying this revision---including the three
analyses added here: candidate-pool sensitivity, the load-uncertainty Monte-Carlo, and
the converter-loss post-processing---is the archived release v1.3-ieee-access-revision under the concept DOI above, which is
the single deposit that reproduces the results reported in this article.} The per-script roles and the artifacts checked
...
\begin{IEEEbiography}[{\includegraphics[width=1in,height=1.25in,clip,keepaspectratio]{skang.jpg}}]{SEUNGWAN KANG}
