...
operational constraint under high penetration of renewable generation and
electrified demand, particularly on unbalanced feeders where phase asymmetry
...
\cite{estatcom2024egwo}. A metaheuristic ranking alone does not bound the distance
to the global optimum \cite{wolpert1997nfl}; the present paper therefore selects the
final design by enumerating and sizing all candidate three-device supports rather
than by a single solver's global search. An auxiliary five-optimizer
...
\cite{zeraati2018bess,kabir2014pvbess,molina2011statcom_ess}. These works
...
Building on established distribution-network and power-system modeling
\cite{baran1989reconfig,sallam2011distribution,kundur1994stability}, three gaps
motivate this work: (i) when raw node-voltage outputs are used without
...
\midrule
\cite{statcom2025cmes}        & Balanced radial test systems      & No  & Bus magnitude (not explicitly reported) & Yes & No \\
