...
independent converter current limits are outside the planning model. Its robustness to
the candidate-pool size and to
phase-independent, spatial load uncertainty, and the effect of converter switching
...
buses changes neither the minimum device count nor the least-capacity support.}
...
\begin{table}[!t]
	\caption{\rev{Maximum Sequence VUF and NEMA Line-Voltage-Unbalance Rate (LVUR) over the
		24-Hour Profile (IEEE 37-Node, \mbox{\SI{1041.3}{\kvar}} Design)}}
	\label{tab:vuf}
	\centering
	\renewcommand{\arraystretch}{1.2}
	\setlength{\tabcolsep}{4pt}
...
\caption{\rev{Phase-independent and spatial Monte-Carlo (\mbox{$N=2000$}) at the
stressed spread \mbox{$(\sigma_\phi,\sigma_\ell)=(0.06,0.08)$}.
...
sensitivity, not a full stochastic validation---\rev{the phase-independent and
spatial Monte-Carlo study described next removes the single-multiplier restriction,
...
\rev{Second, a phase-independent and spatial Monte-Carlo study removes the
single-multiplier restriction by perturbing every load independently in space and
phase. Each of \mbox{$N=2000$} draws multiplies load \mbox{$\ell$} by}
\begin{equation}
\reveq{m_\ell = 1 + e_{\phi(\ell)} + e_\ell,\qquad
e_\phi\sim\mathcal N(0,\sigma_\phi^2),\ \ e_\ell\sim\mathcal N(0,\sigma_\ell^2)}
\label{eq:mcerr}
\end{equation}
\rev{with three phase-systematic terms \mbox{$e_\phi$} (one per phase, drawn
independently of one another) and a per-load idiosyncratic term \mbox{$e_\ell$},
sampled about base points drawn from the daily profile. The two terms are independent
\emph{draws}, but loads are not mutually independent: those assigned to the same phase
group share \mbox{$e_{\phi}$} and are therefore correlated, which is what produces
group-wise unbalance rather than a uniform scaling. Each load object in the OpenDSS
model carries one multiplier applied to its $P$ and $Q$ together; the phase index
\mbox{$\phi(\ell)$} is taken from the first node of the load's bus connection, so for a
delta-connected load the multiplier is attached to that terminal as a whole and the
three branches of a three-phase object are not perturbed separately. Draws use a fixed
seed (2026) with \mbox{$N=2000$} per uncertainty level, multipliers are floored at
\mbox{0.05} to exclude non-physical negative load, and every controller is evaluated on
the \emph{same} draw so the comparison is paired. Over the installed
\mbox{$\{735,740,741\}$} design the connection-voltage band is violated in
\mbox{1.95\%} of draws at \mbox{$(\sigma_\phi,\sigma_\ell)=(0.04,0.05)$} and
\mbox{2.75\%} at the stressed \mbox{$(0.06,0.08)$}; the worst line-to-line minima are
\mbox{0.934} and \mbox{0.925} pu---the latter \mbox{0.025} pu below the assessment
limit---with the fifth-percentile minimum at or above \mbox{0.955} pu, and the worst
sequence VUF stays at
\mbox{4.85\%} and \mbox{5.23\%}, consistent with the deterministic audit. Three
controls---fixed full output, an open-loop forecast schedule that applies the
hour's pre-computed set-points without observing the realized error, and the reactive
local proxy---return the \emph{same} violation probability and worst voltage. This is
not a modelling artefact: auditing the individual draws shows that every violating
draw occurs in the evening hours 18--20 (load multiplier \mbox{1.26--1.34}) and that in
those draws the schedule and the proxy both sit at full output
(\mbox{1041.3 kvar}), so all three controls apply the same injection and reach the same
voltage. Away from that tail the controls do differ---roughly \mbox{45\%} of all draws
give a different minimum voltage under fixed and proxy control---and over the day they
differ in efficiency (mean injected reactive power \mbox{1041}, \mbox{1007}, and
\mbox{999} kvar and mean feeder loss \mbox{123.4}, \mbox{122.0}, and \mbox{121.7} kW at
the nominal spread, Fig.~\mbox{\ref{fig:mc}}). Feasibility in the binding tail is
therefore governed by the installed capacity rather than the control law. The residual
violation rate is reported as observed; the design was sized on a deterministic peak
snapshot and is not certified against a chance constraint.}
...
overloaded snapshot is deliberate rather than inconsistent: $\lambda=1.34$ is an
upper stress case, and reconductoring the head line would raise its ampacity while
lowering its series impedance, relieving the overload and reducing the voltage drop
that feeds the remote terminals. The capacity reported here is therefore the support
required \emph{before} any such reinforcement, and re-optimizing the two in sequence
is expected to reduce rather than increase it.}
...
24-hour dispatch. For a device of nameplate rating $S_r$ delivering $S$,}
\begin{equation}
...
$0.4\,L_{\mathrm{tot}}S_r$ remains at $S=0$. Each device is taken at its hardware
nameplate $S_r=\mbox{450 kvar}$ (three devices, \mbox{1350 kvar} installed), not at the
dispatched total, and $S/S_r$ is used as a proxy for the current ratio, which neglects
the terminal-voltage variation. The total loss at rating $L_{\mathrm{tot}}$ is swept
...
holding full output continuously is the first strategy to turn net-negative---but the
net benefit is not positive across the whole swept range: within \mbox{1--2.5\%} the
...
proxy for a full four-quadrant energy-storage converter.
...
held-out scenarios is the natural next step. \rev{The phase-specific and spatial
components are addressed directly by the phase-independent Monte-Carlo of
Section~\mbox{\ref{sec:robust}} (\mbox{2--3\%} band violations), leaving correlated
temporal and DER-coupled variation and scenario-based sizing as the remaining gap.} The voltage dependence $Q_{\mathrm{cap}}\propto V^2$ in \eqref{eq:cap} is a general
...
Section~\ref{sec:operation}: the band is held in all \num{27} settings, while the
...
left the minimum device count and the best known least-capacity support unchanged,
and a phase-independent, spatial load Monte-Carlo violated the band in \mbox{2--3\%}
of draws, so the selection is insensitive to the pool cut-off while the residual
...
\mbox{1.5\%}, so controllable dispatch is what keeps the net benefit positive over the
swept range.} Applied with native controls active, the same
...
the converter-loss post-processing---is the archived release v1.2-ieee-access-revision under the concept DOI above, which is
