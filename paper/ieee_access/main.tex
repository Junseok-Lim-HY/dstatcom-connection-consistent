...
123-node feeder. \rev{Its robustness to the candidate-pool size and to
...
\begin{figure}[!t]
	...
	\caption{Largest worst line-to-line minimum $\hat v_{\mathrm{best}}$ found
		\eqref{eq:vbest} for one- and two-device supports (bars); neither reaches the
		\SI{0.9562}{pu} target. The three-device point (diamond) is the canonical design's
		\emph{operating} minimum---a feasibility witness, not a $\hat v_{\mathrm{best}}$.}
	\label{fig:w4bars}
\end{figure}
...
\begin{figure}[!t]
	...
	\caption{\rev{Candidate-pool and device-count sensitivity. (a) No one- or two-device
		support is feasible over all 37 eligible buses, fixing the minimum device count at
		three; (b) enlarging the candidate pool from 10 to 20 buses expands the feasible
		three-device set (119, 359, and 852 combinations) but leaves the least-capacity
		support and its 1043.1-kvar rating unchanged.}}
\label{fig:poolsens}
\end{figure}
...
buses, no one- or two-device support was feasible, so the minimum feasible count is
three independently of the pool. Second, re-running the full three-device
enumeration over enlarged pools of the 10, 15, and 20 weakest buses returns the
\emph{same} least-capacity support in all three cases---\mbox{$\{711,735,740\}$} at
\mbox{1043.1 kvar} under this study's wider three-seed budget, within the few-kvar
near-equivalent family of the canonical \mbox{$\{735,740,741\}$} design
(\mbox{1041.3 kvar}, this section). The number of feasible three-device
combinations grows from 119 to 359 to 852 as the pool widens, yet the optimum does
not move and the installed capacity does not fall, because the additional buses are
electrically weaker and non-binding. The ten-bus low-voltage cluster therefore
already contains the binding support, and the reported design is not an artefact of
the pool cut-off.}
...
\SI{1041.3}{\kvar} Design; Single Connection-Consistent Evaluator). Worst minima
occur at the peak hour and maxima at light load; all binding terminals are
line-to-line (the binding pair is phases $c$--$a$ at bus~740).}
\label{tab:canonical}
\centering
\renewcommand{\arraystretch}{1.2}
\setlength{\tabcolsep}{3pt}
...
\caption{\rev{Phase-independent and spatial Monte-Carlo (\mbox{$N=2000$}).
(a) Distribution of the minimum line-to-line voltage under the reactive local proxy
at the stressed spread; the \mbox{0.95}-pu band is breached only in a shallow
\mbox{2.75\%} tail. (b) The three controls are indistinguishable on feasibility
(identical worst voltage) and differ only in mean injected reactive power and mean
feeder loss.}}
\label{fig:mc}
\end{figure*}
...
\label{eq:mcerr}
\end{equation}
\rev{with three independent phase-systematic terms \mbox{$e_\phi$} (one per phase)
and a per-load idiosyncratic term \mbox{$e_\ell$}, sampled about base points drawn
from the daily profile, so different phases and feeder locations move independently
and create genuine unbalance rather than a uniform scaling. Over the installed
...
\mbox{2.75\%} at the stressed \mbox{$(0.06,0.08)$}; the violations are shallow
(worst line-to-line minimum \mbox{0.934} and \mbox{0.925} pu, fifth-percentile
minimum at or above \mbox{0.955} pu) and the worst sequence VUF stays at
\mbox{4.85\%} and \mbox{5.23\%}, consistent with the deterministic audit. Three
controls---fixed full output, an open-loop forecast schedule, and the reactive
local proxy---return the \emph{same} violation probability and worst voltage,
because the binding stressed draws saturate every control to full output; they
differ only in efficiency (mean injected reactive power \mbox{1041}, \mbox{1007},
and \mbox{999} kvar and mean feeder loss \mbox{123.4}, \mbox{122.0}, and
\mbox{121.7} kW at the nominal spread, Fig.~\mbox{\ref{fig:mc}}). Phase-independent
and spatial uncertainty therefore leaves feasibility governed by the installed
capacity rather than the control law, and the residual few-percent violation is the
expected cost of deterministic peak sizing.}
...
\begin{figure}[!t]
	...
	\caption{\rev{Net daily energy (feeder-loss saving minus converter switching and
			conduction loss) versus converter loss at rating. Coordinated dispatch, which injects
			the least reactive energy, stays net-positive to the highest loss level (break-even
			``BE'' in the legend), while fixed full output turns net-negative first.}}
	\label{fig:vscloss}
\end{figure}
...
by the power flow. To gauge their effect, a three-term device-loss model---a fixed
switching term plus current-proportional and current-squared conduction terms,
apportioned to each device in proportion to its rating---is applied post hoc to the
24-hour dispatch, with the converter loss at rating swept from 1 to 2.5\%
(Fig.~\mbox{\ref{fig:vscloss}}). Because the coordinated and proxy laws inject far
...
only \mbox{1.29\%} fixed. Accounting for converter losses therefore \emph{strengthens}
the case for controllable dispatch: the same controllability that lowers feeder loss
also limits the converter's own loss, whereas holding full output continuously is the
first strategy to turn net-negative. The absolute figures depend on the assumed loss
...
as are measurement delay, filtering, converter inner-loop dynamics, \rev{switching
harmonics, and output-filter design}.
...
left the minimum device count and the least-capacity support unchanged; a
phase-independent, spatial load Monte-Carlo violated the band in only \mbox{2--3\%}
of draws; and once converter switching and conduction losses are included,
controllable dispatch remained net-beneficial (break-even converter loss
\mbox{1.3--2.5\%}), so the design is robust to the pool cut-off and to the
single-multiplier stress and its dispatch benefit survives realistic converter
losses.} Applied with native controls active, the same
...
\texttt{v1.0-ieee-access-submission}. \rev{The three additional analyses introduced
in revision---candidate-pool sensitivity, phase-independent Monte-Carlo, and
converter-loss post-processing---are included in the same repository under Release
\texttt{v1.1-ieee-access-revision}.} The per-script roles and the artifacts checked
